...
import {
  ...
  type FinalidadKey,
  type Upgrade,
} from "@/data/configurador-catalog";
...
export default function ConfiguradorMovara({
    ...
    try {
      ...
      const res = await fetch("/api/pedido", {
        ...
        body: JSON.stringify({
          ...
          tipoCliente,
          razonSocial: tipoCliente === "empresa" ? razonSocial.trim() : undefined,
          nombreContacto: tipoCliente === "empresa" ? nombreContacto.trim() : undefined,
          modelo,
          finalidad,
          provincia,
          localidad,
          ...
          lavarropas,
          materiales,
          upgrades: Array.from(upgradesSeleccionados),
  ...
  function goNext() {
    if (step < 6) {
      ...
      setWaMessage(msg);
      setShowResult(true);
      // No bloquea el mensaje de WhatsApp si el guardado falla — el lead
      // igual tiene que poder llegar por WhatsApp.
      void submitPedido();
    }
  }
